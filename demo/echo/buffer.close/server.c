...
////////////////////////////////////////////////////////////////////////////////
/// An echo server that buffers the ingoing requests until '\n' or '\0'. But,
/// this one forgets to reset these buffers upon a disconnection (because who
/// drops their connection mid-request?).
////////////////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < nopen; ++i) {
      ...
      if (nread < 0) { // Error?
        ...
        while (buffer_idx < nread) {
          ...
          recv_size[i] += buffer_idx;
          buffer_idx += 1;

          // Send message back if complete!
          if (eom_idx != -1) {
            const int nwrite = send(fd, recv_buff[i], eom_idx+1, 0);
            recv_size[i] = 0;
            if (nwrite < 0) {
